...
export default function BlogPostView({ postId, backHref = "/blog" }: { postId: string; backHref?: string }) {
  ...
  useEffect(() => {
    ...
    if (post.id === "heap-overflow-elf-hunting") return; // DeepDivePost handles its own
  ...
  if (!post) return null;
  if (post.id === "heap-overflow-elf-hunting") {
    return <DeepDivePost onBack={() => window.location.assign(backHref)} />;
  }
  return (
    <article className="max-w-3xl mx-auto" id="blog-reader-post">
      <a href={backHref} className="inline-flex items-center gap-2 text-[13px] mb-6 px-3 py-1.5 rounded-full border hover:bg-[var(--card-inner)] transition-colors" style={{ color: 'var(--muted-foreground)', borderColor: 'var(--card-border)', fontFamily: 'var(--font-sans)' }}>
        ← Back to index
      </a>
      <header className="mb-8">
        <h1 className="text-[28px] md:text-[34px] font-semibold tracking-tight leading-[1.05]" style={{ color: 'var(--foreground)', fontFamily: 'var(--font-title)' }}>{post.title}</h1>
        <div className="flex flex-wrap items-center gap-3 mt-4 text-[13px]" style={{ color: 'var(--muted-foreground)', fontFamily: 'var(--font-sans)' }}>
          ...
          <span>·</span><span>{post.date}</span><span>·</span><span>{post.readTime}</span><span>·</span><span className="px-2 py-0.5 rounded-full border text-[11px]" style={{ borderColor: 'var(--card-border)', background: 'var(--card-inner)' }}>#{post.category.toLowerCase()}</span>
        </div>
        <p className="mt-4 text-[15px] leading-relaxed italic" style={{ color: 'var(--muted-foreground)', fontFamily: 'var(--font-sans)' }}>"{post.description}"</p>
      </header>
      <div className="w-full rounded-[12px] overflow-hidden relative mb-8" style={{ border: '1px solid var(--card-border)', background: 'var(--card-inner)' }}>
        <div className="w-full aspect-[200/96] rounded-[10px] overflow-hidden relative">
          <img src={post.image || "/images/pixel-clouds-blog.jpg"} alt={post.title} className="w-full h-full object-cover" style={{ objectPosition: 'center 28%', display: 'block' }} />
        </div>
      </div>
      <div className="prose max-w-none text-[15px] leading-7 space-y-4" style={{ color: 'var(--foreground)', fontFamily: 'var(--font-sans)' }}>
        {(() => {
          let lastHeadingId = "";
          const _parts = post.content.split(/(```[\s\S]*?```)/g);
      ...
      <footer className="mt-10 pt-6 flex flex-wrap gap-2 items-center" style={{ borderTop: '1px solid var(--card-border)' }}>
        ...
        {post.tags.map((t) => <span key={t} className="px-2.5 py-1 rounded-full text-[12px] border" style={{ background: 'var(--card-inner)', borderColor: 'var(--card-border)', color: 'var(--muted-foreground)', fontFamily: 'var(--font-mono)' }}>#{t}</span>)}
      </footer>
    </article>
  );
}
