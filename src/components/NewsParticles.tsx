...
// Port of the reference BannerParticles leaf effect, recoloured cream:
// wind-blown tumbling leaf sprites at full visibility.
const settings = {
  minWind: 1.2,
  maxWind: 4.0,
  minSize: 6,
...
function createLeafImage() {
  ...
  tCtx.fillStyle = "#FFF5DD";
  tCtx.fill();

  tCtx.strokeStyle = "#c9c2b3";
  tCtx.lineWidth = 1;
  tCtx.stroke();
...
export default function NewsParticles() {
  ...
  useEffect(() => {
    ...
    updateCache();
    const leafImage = createLeafImage();
    if (!leafImage) return;
    ...
    class Particle {
      ...
      draw() {
        ...
        ctx.drawImage(leafImage, -this.width / 2, -this.height / 2, this.width, this.height);
        ctx.restore();
      }
    }
    ...
    // NOTE: <astro-island> wrappers are display:contents (no box), so climb
    // to the nearest ancestor that actually has dimensions (the card).
    const findBox = () => {
      let el: HTMLElement | null = canvas.parentElement;
      while (el) {
        if (el.clientWidth > 0 && el.clientHeight > 0) return el;
        el = el.parentElement;
      }
      return null;
    };
    ...
    const initParticles = () => {
      ...
      const count = Math.min(
        ...
        Math.max(16, Math.round((width * height) / 2500)),
      );
    ...
    const startAnimation = () => {
      if (isUnmounted || isRunning || !isInViewport || !isPageVisible || prefersReducedMotion)
        return;
      isRunning = true;
      animationFrameId = requestAnimationFrame(animate);
    };
